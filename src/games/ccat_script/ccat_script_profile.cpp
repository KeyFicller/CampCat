...
void from_json(const nlohmann::json &_j, ccat_script_profile &_p) {
  ...
  if (_j.contains("images_root") && _j["images_root"].is_string()) {
    _p.images_root_rel = _j["images_root"].get<std::string>();
  }
}
...
ccat_script_profile::images_base(const std::filesystem::path &_config_home) const {
  if (!images_root_rel.empty()) {
    return _config_home / images_root_rel;
  }
  std::error_code ec;
  const std::filesystem::path script_path = _config_home / source_rel;
